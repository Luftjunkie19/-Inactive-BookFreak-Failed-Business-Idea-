...
function UserChatBubble({item, user, condition, images}: Props) {
  return (
    <div key={item.id} className={`chat ${condition ? 'chat-start' : 'chat-end'}`}>
         <div className="chat-image avatar">
           <div className="w-10 rounded-full">
            <Image  width={40} height={60} src={item.sender.photoURL} alt='' className='w-10 h-10 rounded-full'/>
           </div>
         </div>
         <div className="chat-header flex items-center gap-1">
          <p className='text-white text-xs'>{item.sender.nickname}, </p>
