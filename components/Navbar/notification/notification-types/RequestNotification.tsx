...
function RequestNotification({competitionData, requestToJoin, clubData,isRead, sentAt, competitionInvitation, clubInvitation, senderId, senderNickname }: Props) {
  return (
  ...
  {requestToJoin && requestToJoin.competition &&   <Image src={requestToJoin.competition.competitionLogo} alt='' width={50} height={50} className='w-12 h-12 object-cover rounded-full'/>}
    <div className="flex flex-col self-start flex-1 gap-1">
      <div className="flex gap-2 justify-between items-center py-1">
            ...
            {requestToJoin.club && <>
              <p className=' text-xs'>{requestToJoin.club.clubName}</p>
