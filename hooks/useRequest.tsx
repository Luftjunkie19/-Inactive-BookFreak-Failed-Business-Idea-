        ...
        if (user) {
            const userDocument = await fetch('/api/supabase/user/get', {
                ...
                body: JSON.stringify({
                    id: user && user.id,
                    include: {
                        BookLover: { include: { user: true, Book: true } }, bookShelfs: {
                            include: { user: true, books: true }, booksInRead: {
                                include: {
                                    book: true,
                                }
                            }
            ...
            await fetch('/api/supabase/request/create', {
                ...
                body: JSON.stringify({
                    data: {
                        userId: user.id,
                        clubId: type === 'club' && id,
                        competitionId: type === 'competition' && id,
                        ...
                        requestType: 'This is request to the club',
                        isAccepted: false,
                    }
                })
            });
